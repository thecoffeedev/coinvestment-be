...
class TestHoldingPeriod(unittest.TestCase):
    ...
    def test_holdingPeriod_must_throw_error_when_none(self):
        with self.assertRaises(ValueError):
            self.newUser.setHoldingPeriod(None)

    def test_holdingPeriod_must_not_be_none_when_set(self):
        self.newUser.setHoldingPeriod(5)
        self.assertIsNotNone(self.newUser.getHoldingPeriod())

    def test_holdingPeriod_must_not_be_str(self):
        with self.assertRaises(TypeError):
            self.newUser.setHoldingPeriod("12345678901234567890")

    def test_holdingPeriod_must_not_be_zero(self):
        with self.assertRaises(ValueError):
            self.newUser.setHoldingPeriod(0)

    def test_holdingPeriod_must_not_be_a_negative_int(self):
        with self.assertRaises(ValueError):
            self.newUser.setHoldingPeriod(-1)

    def test_holdingPeriod_must_not_be_a_negative_float(self):
        with self.assertRaises(TypeError):
            self.newUser.setHoldingPeriod(-0.01)

    def test_holdingPeriod_must_not_be_a_positive_float_whole_number(self):
        with self.assertRaises(TypeError):
            self.newUser.setHoldingPeriod(1.00)

    def test_holdingPeriod_must_not_be_a_negative_float_whole_number(self):
        with self.assertRaises(TypeError):
            self.newUser.setHoldingPeriod(-1.00)

    def test_holdingPeriod_must_not_be_a_true_bool(self):
        with self.assertRaises(TypeError):
            self.newUser.setHoldingPeriod(True)

    def test_holdingPeriod_must_not_be_a_false_bool(self):
        with self.assertRaises(TypeError):
            self.newUser.setHoldingPeriod(False)

    def test_holdingPeriod_must_be_int(self):
        self.newUser.setHoldingPeriod(1)
        self.assertIsInstance(self.newUser.getHoldingPeriod(), int)

    def test_HoldingPeriod_must_be_set_correctly_at_initilization_when_provided(self):
